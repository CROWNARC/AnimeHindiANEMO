...
export default function AnimeDetailPage({ params }: AnimeDetailPageProps) {
  ...
  const [isActive, setIsActive] = useState(false) // Declare isActive variable
  const [notFound, setNotFound] = useState(false)

  useEffect(() => {
    const fetchAnime = async () => {
      // Check if the anime ID is in the list of available anime
      const { ALL_AVAILABLE_ANIME_TMDB_IDS } = await import('@/lib/anime-mapping')
  ...
  if (notFound) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-foreground">
        ...
        <p className="text-xl mb-6">This anime is not available on our website.</p>
        <Link href="/">
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <div className="relative z-10 max-w-4xl text-white">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            {selectedSeasonNumber && selectedSeasonEpisodes.length > 0 && (
              ...
              >
                <Button className="glass-effect text-lg px-6 py-3 w-full sm:w-auto">Watch First Episode</Button>
              </Link>
            )}
      ...
      <main className="flex-1 container mx-auto px-4 py-8 md:py-12 anime-detail-container">
        {/* Anime Details */}
        <section className="mb-12 grid md:grid-cols-[300px_1fr] gap-8 anime-detail-section">
